...
# Updated: Recursively collect image files in all subdirectories
...
test_covid = get_image_files(args.test_covid_path)
test_non = get_image_files(args.test_non_covid_path)
...
print("First 10 non-COVID probabilities:", non_prob[:10])
...
# Save probabilities
pickle.dump(covid_prob, open(f"./results/covid_prob_{model_filename}.p", "wb"))
pickle.dump(non_prob, open(f"./results/non_prob_{model_filename}.p", "wb"))
...
covid_list = covid_pred.flatten().tolist()
covid_count = [(x, covid_list.count(x)) for x in set(covid_list)]

non_list = non_pred.flatten().tolist()
...
print("y_test distribution:", np.unique(y_test_list, return_counts=True))
print("y_pred distribution:", np.unique(y_pred_list, return_counts=True))
...
plt.subplot(212)
plt.hist(non_prob, bins, color='green', histtype='bar', label='Probabilities of Non-COVID Samples')
plt.legend(loc='upper center')
plt.xlabel("Predicted Probability")
plt.ylabel("Frequency")

plt.tight_layout()  # ← fixes the overlap issue
...
plt.figure()
pyplot.plot(ns_fpr_res18, ns_tpr_res18,  color='darkgreen', linewidth=2,   label='ResNet50,        AUC= %.3f' %auc_res18)
pyplot.ylim([0,1.05])
